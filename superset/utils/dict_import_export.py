...
from superset.connectors.graphql.models import GraphQLEndPoint
from superset.models.core import Database


DATABASES_KEY = 'databases'
DRUID_CLUSTERS_KEY = 'druid_clusters'
GRAPHQL_ENDPOINTS_KEY = 'graphql_endpoints'
...
def export_schema_to_dict(back_references):
    ...
    clusters = [DruidCluster.export_schema(recursive=True,
                include_parent_ref=back_references)]
    endpoints = [GraphQLEndPoint.export_schema(recursive=True,
                 include_parent_ref=back_references)]
    data = dict()
    if databases:
    ...
    if endpoints:
        data[GRAPHQL_ENDPOINTS_KEY] = endpoints
    return data
...
def export_to_dict(session,
    ...
    eps = session.query(GraphQLEndPoint)
    endpoints = [endpoint.export_to_dict(recursive=recursive,
                include_parent_ref=back_references,
                include_defaults=include_defaults) for endpoint in eps]
    logging.info('Exported %d %s', len(endpoints), GRAPHQL_ENDPOINTS_KEY)
    data = dict()
    if databases:
        data[DATABASES_KEY] = databases
    if clusters:
...
def import_from_dict(session, data, sync=[]):
    ...
    if isinstance(data, dict):
        ...
        logging.info('Importing %d %s',
                     len(data.get(GRAPHQL_ENDPOINTS_KEY, [])),
                     GRAPHQL_ENDPOINTS_KEY)
        for datasource in data.get(GRAPHQL_ENDPOINTS_KEY, []):
            GraphQLEndPoint.import_from_dict(session, datasource, sync=sync)
