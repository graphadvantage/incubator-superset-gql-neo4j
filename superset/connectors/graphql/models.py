...
class GraphQLEndPoint(Model, AuditMixinNullable, ImportMixin):
    ...
    def get_datasources(self):
        endpoint = self.get_base_broker_url() + '/graphqldatasources'
    ...
    def get_druid_version(self):
        endpoint = self.get_base_url(
            self.broker_host, self.broker_port) + '/graphqlstatus'
        auth = requests.auth.HTTPBasicAuth(self.broker_user, self.broker_pass)
    ...
    def refresh(self, datasource_names, merge_flag, refreshAll):
        ...
        session = db.session
        ds_list = (
            session.query(GraphQLDatasource)
            .filter(GraphQLDatasource.cluster_name == self.cluster_name)
            .filter(GraphQLDatasource.datasource_name.in_(datasource_names))
        )
        ds_map = {ds.name: ds for ds in ds_list}
        for ds_name in datasource_names:
            datasource = ds_map.get(ds_name, None)
            if not datasource:
                datasource = GraphQLDatasource(datasource_name=ds_name)
        ...
        for i in range(0, len(ds_refresh)):
            ...
            if cols:
                col_objs_list = (
                    session.query(GraphQLColumn)
                    .filter(GraphQLColumn.datasource_id == datasource.id)
                    .filter(GraphQLColumn.column_name.in_(cols.keys()))
                )
...
class GraphQLColumn(Model, BaseColumn):
    """ORM model for storing Druid datasource column metadata"""

    __tablename__ = 'graphql_columns'
    __table_args__ = (UniqueConstraint('column_name', 'datasource_id'),)

    datasource_id = Column(
        Integer,
        ForeignKey('graphql_datasources.id'))
    ...
    def refresh_metrics(self):
        ...
        dbmetrics = (
            db.session.query(GraphQLMetric)
            .filter(GraphQLMetric.datasource_id == self.datasource_id)
            .filter(GraphQLMetric.metric_name.in_(metrics.keys()))
        )
    ...
    def import_obj(cls, i_column):
        def lookup_obj(lookup_column):
            return db.session.query(GraphQLColumn).filter(
                GraphQLColumn.datasource_id == lookup_column.datasource_id,
                GraphQLColumn.column_name == lookup_column.column_name).first()

        return import_datasource.import_simple_obj(db.session, i_column, lookup_obj)


class GraphQLMetric(Model, BaseMetric):

    """ORM object referencing Druid metrics for a datasource"""

    __tablename__ = 'graphql_metrics'
    ...
    def import_obj(cls, i_metric):
        def lookup_obj(lookup_metric):
            return db.session.query(GraphQLMetric).filter(
                GraphQLMetric.datasource_id == lookup_metric.datasource_id,
                GraphQLMetric.metric_name == lookup_metric.metric_name).first()
        return import_datasource.import_simple_obj(db.session, i_metric, lookup_obj)


graphqldatasource_user = Table(
    'graphqldatasource_user', metadata,
    Column('id', Integer, primary_key=True),
    Column('user_id', Integer, ForeignKey('ab_user.id')),
    Column('datasource_id', Integer, ForeignKey('graphql_datasources.id')),
)


class GraphQLDatasource(Model, BaseDatasource):

    """ORM object referencing Druid datasources (tables)"""

    __tablename__ = 'graphql_datasources'
    __table_args__ = (UniqueConstraint('datasource_name', 'cluster_name'),)

    type = 'graphql'
    query_language = 'json'
    cluster_class = GraphQLEndPoint
    metric_class = GraphQLMetric
    column_class = GraphQLColumn
    owner_class = security_manager.user_model

    baselink = 'graphqldatasourcemodelview'
    ...
    cluster_name = Column(
        String(250), ForeignKey('graphql_endpoints.cluster_name'))
    cluster = relationship(
        'GraphQLEndPoint', backref='datasources', foreign_keys=[cluster_name])
    owners = relationship(owner_class, secondary=graphqldatasource_user,
                          backref='graphqldatasources')
    ...
    def import_obj(cls, i_datasource, import_time=None):
        ...
        def lookup_datasource(d):
            return db.session.query(GraphQLDatasource).filter(
                GraphQLDatasource.datasource_name == d.datasource_name,
                GraphQLEndPoint.cluster_name == d.cluster_name,
            ).first()

        def lookup_cluster(d):
            return db.session.query(GraphQLEndPoint).filter_by(
    ...
    def sync_to_db_from_config(
        ...
        col_objs = (
            session.query(GraphQLColumn)
            .filter(GraphQLColumn.datasource_id == datasource.id)
            .filter(GraphQLColumn.column_name.in_(dimensions))
        )
        ...
        metric_objs = (
            session.query(GraphQLMetric)
            .filter(GraphQLMetric.datasource_id == datasource.id)
            .filter(GraphQLMetric.metric_name.in_(
    ...
    def recursive_get_fields(_conf):
        ...
        if _field:
            field_names += GraphQLDatasource.recursive_get_fields(_field)
        if _fields:
            for _f in _fields:
                field_names += GraphQLDatasource.recursive_get_fields(_f)
    ...
    def resolve_postagg(postagg, post_aggs, agg_names, visited_postaggs, metrics_dict):
        ...
        required_fields = set(
            GraphQLDatasource.recursive_get_fields(mconf) +
        ...
        if len(required_fields) > 0:
            missing_postaggs = GraphQLDatasource.find_postaggs_for(
            ...
            for missing_postagg in missing_postaggs:
                GraphQLDatasource.resolve_postagg(
                    missing_postagg, post_aggs, agg_names, visited_postaggs, metrics_dict)
        post_aggs[postagg.metric_name] = GraphQLDatasource.get_post_agg(postagg.json_obj)
    ...
    def metrics_and_post_aggs(metrics, metrics_dict, druid_version=None):
        ...
        aggs = GraphQLDatasource.get_aggregations(
            metrics_dict,
            saved_agg_names,
            adhoc_agg_configs,
        )
            ...
            for unused, row in df.iterrows():
                ...
                for dim in dimensions:
                    ...
                    if isinstance(dim, dict) and 'extractionFn' in dim:
                        (col, extraction_fn) = GraphQLDatasource._create_extraction_fn(dim)
    ...
    @staticmethod
    def graphql_type_from_adhoc_metric(adhoc_metric):
    ...
    def get_aggregations(metrics_dict, saved_metrics, adhoc_metrics=[]):
        ...
        for adhoc_metric in adhoc_metrics:
            aggregations[adhoc_metric['label']] = {
                ...
                'type': GraphQLDatasource.graphql_type_from_adhoc_metric(adhoc_metric),
    ...
    def run_query(  # noqa / druid
        ...
        aggregations, post_aggs = GraphQLDatasource.metrics_and_post_aggs(
            metrics,
            metrics_dict)
        ...
        qry = dict(
            ...
            granularity=GraphQLDatasource.granularity(
                granularity,
                timezone=timezone,
        ...
        filters = GraphQLDatasource.get_filters(filter, self.num_cols, columns_dict)
        if filters:
            qry['filter'] = filters
        ...
        if columns:
            ...
            if timeseries_limit_metric:
                ...
                aggs_dict, post_aggs_dict = GraphQLDatasource.metrics_and_post_aggs(
    ...
    def query(self, query_obj):
        ...
        time_offset = GraphQLDatasource.time_offset(query_obj['granularity'])
    ...
    def get_filters(cls, raw_filters, num_cols, columns_dict):  # noqa
        ...
        for flt in raw_filters:
            ...
            extraction_fn = None
            if dim_spec and 'extractionFn' in dim_spec:
                (col, extraction_fn) = GraphQLDatasource._create_extraction_fn(dim_spec)
...
sa.event.listen(GraphQLDatasource, 'after_insert', security_manager.set_perm)
sa.event.listen(GraphQLDatasource, 'after_update', security_manager.set_perm)
